...
var PathScene = load("res://PathScene.tscn")
var deltasum = 0.0
...
func _physics_process(delta):
	...
	if !ended:
		...
		deltasum += delta
		if deltasum > 0.5:
			curve_path.add_point(get_global_transform().origin)
			deltasum = 0.0
...
func get_input(delta):
	...
	if Input.is_action_pressed("end"):
		...
		ended = true
		var PathScene_inst = PathScene.instance()
		main.add_child(PathScene_inst)
		get_node("/root/main/Path").set_curve(curve_path)
		get_node("/root/main/Camera").target_path = "/root/main/Path/PathFollow/segment"
